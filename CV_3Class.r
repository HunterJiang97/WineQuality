...
model.opttree <- function(train, test){
  ...
  opt <- max(cv_tree$size[which.min(cv_tree$dev)], 2)
  model_opt <- prune.tree(model_tree, best = opt)
...
Stat_OR <- function(YList, YhatList){
  ...
  for (ii in 1:n){
    for (jj in 1:nn){
      ...
      tbl[1,1] <- tbl[1,1] + length(which(Y == 1 & Yhat == 1))
      tbl[1,2] <- tbl[1,2] + length(which(Y == 1 & Yhat == 2))
      tbl[1,3] <- tbl[1,3] + length(which(Y == 1 & Yhat == 3))
      tbl[2,1] <- tbl[2,1] + length(which(Y == 2 & Yhat == 1))
      tbl[2,2] <- tbl[2,2] + length(which(Y == 2 & Yhat == 2))
      tbl[2,3] <- tbl[2,3] + length(which(Y == 2 & Yhat == 3))
      tbl[3,1] <- tbl[3,1] + length(which(Y == 3 & Yhat == 1))
      tbl[3,2] <- tbl[3,2] + length(which(Y == 3 & Yhat == 2))
      tbl[3,3] <- tbl[3,3] + length(which(Y == 3 & Yhat == 3))
    }
  }
...
for (ii in 1:K){
  ...
  CVtrain2[[ii]]$quality[CVtrain2[[ii]]$quality >=7] <- 3 
  CVtest2[[ii]]$quality[CVtest2[[ii]]$quality <=5] <- 1 
  CVtest2[[ii]]$quality[CVtest2[[ii]]$quality ==6] <- 2
  CVtest2[[ii]]$quality[CVtest2[[ii]]$quality >=7] <- 3
  train <- CVtrain2[[ii]]
  test <- CVtest2[[ii]]
